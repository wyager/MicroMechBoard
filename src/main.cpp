...
int main(void)
{
    ...
    //LED 1: PD7
    DDRD |= 1 << 7;//Enable output mode for PD7
    PORTD |= 1 << 7; //Turn on LED 1
    //LED 2: PB5
    DDRB |= 1 << 5;//Enable output mode for PB5
    PORTB |= 1 << 5; //Turn on LED 2
    //LED 3: PB6
    DDRB |= 1 << 6;//Enable output mode for PB6
    PORTB |= 1 << 6; //Turn on LED 3
